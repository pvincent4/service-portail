  ...
  def set_current_user( env )
    ...
    if session[:user] && !user_annuaire.nil? # && user_annuaire['profils'].select do |profil| profil['bloque'].nil? end.length > 0
      session[:current_user] = {
        ...
        info: session[:extra],
        is_logged: true,
        avatar: user_annuaire['avatar'].match( /empty$/ ).nil? ? ANNUAIRE[:url].gsub( %r{/api/app}, '' ) + user_annuaire['avatar'] : "#{APP_PATH}/vendor/charte-graphique-laclasse-com/images/avatar_#{user_annuaire['sexe'] == 'F' ? 'feminin' : 'masculin'}.svg",
